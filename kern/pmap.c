...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
		if (i == 0 || i == PGNUM(MPENTRY_PADDR)) {
			...
			pages[i].pp_ref = 1;
			pages[i].pp_link = NULL;
		}else if (i<(((uint32_t)boot_alloc(0) - KERNBASE)>>PGSHIFT)){ //TODO: why
			pages[i].pp_ref = 1;
			pages[i].pp_link = NULL;
		}else{
			pages[i].pp_ref = 0;
...
page_free(struct PageInfo *pp)
{
	...
	{
		panic("pp->pp_ref is not zero or pp->pp_link is not NULL\n");
	}
...
{
	uint16_t va_pdx = PDX(va);
	uint16_t va_ptx = PTX(va);
	uint16_t va_off = PGOFF(va);

	struct PageInfo* pp;
	pte_t* result;
	pde_t* pde = &pgdir[va_pdx];
	if (!(*pde & PTE_P)){
		if (!create)
		{
			return NULL;
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	return pte ? pa2page(PTE_ADDR(*pte)) : NULL;
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (i; i < (uintptr_t) ROUNDUP(va + len, PGSIZE); i += PGSIZE)
	{
		pte_t* pte = pgdir_walk(env->env_pgdir, (void*)i, 0);
		if(!pte || i >= ULIM || ((*pte) & perm) != perm){
			user_mem_check_addr = i > (uintptr_t) va ? i : (uintptr_t) va;
			return -E_FAULT;
		}
	}
	return 0;
